...
class dototransportermodel extends CI_Model{
 ...
 /*           
    public function getDoTransLists($isPending=''){
      ...
      public function getDoTransLists($isPending='',$saleNo){
        
           if($isPending=='Y'){
                $whereClause = " WHERE `do_to_transporter`.`is_sent` ='Y' AND `purchase_invoice_master`.`sale_number`='".$saleNo."' ORDER BY purchase_invoice_master.id DESC";
           }else{
               $whereClause=" WHERE (`do_to_transporter`.`is_sent` IS NULL OR `do_to_transporter`.`is_sent` ='N') AND `purchase_invoice_master`.`sale_number`='".$saleNo."' ORDER BY purchase_invoice_master.id DESC";
           }
              ...
              $sql="SELECT 
                    ...
                    INNER JOIN
                    `purchase_invoice_detail` 
                    ON `purchase_invoice_master`.`id`=`purchase_invoice_detail`.`purchase_master_id` AND `purchase_invoice_master`.`from_where`<>'SB' AND `purchase_invoice_master`.`from_where`<>'STI'
                    LEFT JOIN   `warehouse` ON purchase_invoice_detail.`warehouse_id` = warehouse.`id`
                    INNER JOIN  `garden_master` ON purchase_invoice_detail.`garden_id` = garden_master.`id`
                    INNER JOIN `grade_master` ON `grade_master`.`id` = `purchase_invoice_detail`.`grade_id`
                    LEFT JOIN `do_to_transporter` ON `purchase_invoice_detail`.`id` = `do_to_transporter`.`purchase_inv_dtlid`
    ...
    public function saleNumberList(){
