...
class purchaseinvoicemastermodel extends CI_Model {
     ...
     * @date 31-05-2016
     */
    public function getVchrDataDtl($mastid,$rateType){
   ...
   public function updateVouchrDtl($mastId,$detaildata,$voucherData){
     ...
     $otherndroundoff = $this->getotherChargesVocherDtl($mastId);
         ...
         // For purchase Account Id
           ...
           $voucherDtlPurch['voucher_amount']= $totalPurchaseAmt;
 ...
 */
   ...
   /* public function updatePurchaseMaster($dataArray) {
     ...
     * @date 27/08/2015
     */
    public function insertNewPurchaseDetail($data,$purchType, $bagData, $NormalbagData,$vdata) {
        $masterDataforupdt = array();
        $BagData = array();
        $purType =$purchType;
        ...
        try {
            ...
            $voucherDetailData = $this->getVchrDataDtl($data['purchase_master_id'], $data['rate_type']);
...
/* public function insertNewPurchaseData($pMaster,$searcharray){
   ...
   */
   ...
   public function insertNewPurchaseData($voucherMaster,$searcharray){
     ...
     try {
          ...
          $this->insertintoPurchaseMaster($searcharray,$insertId);
          $insertPId = $this->db->insert_id();
  ...
  public function voucherMasterDtlInsert($insertId,$searcharray){
      ...
      // For purchase Account Id
            $purchaseAmt = $searcharray['txtTeaValue']+$searcharray['txtBrokerageTotal']+$searcharray['txtServiceTax']+$searcharray['txtStampTotal']+
          ...
          //  $detailsData['garden_id'] = 0;
           ...
           // $detailsData['stamp'] = $searcharray['txtStamp'][$i];
            ...
            if($searcharray['purchasetype']=='SB'){
                $this->saveDataDoToTranspoerter($insertPId,$insertdtlId,$searcharray['drplocation'][$i]);
            }
            //if purchase type SB
...
public function getVoucherDtldataFromPMast($pMasterId){
     $sql="SELECT 
        ...
        if ($query->num_rows() > 0) {
            ...
            $data['purchAmt']=$rows->tea_value+$rows->brokerage+$rows->service_tax+$rows->total_cst+$rows->stamp+$rows->round_off+$rows->other_charges;
