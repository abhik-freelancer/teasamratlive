...
class unreleaseddomodel extends CI_Model {
    ...
    public function getDoLists($isPending = '') {
        ...
        $sql = "SELECT 
                    ...
                    ON `purchase_invoice_master`.`id`=`purchase_invoice_detail`.`purchase_master_id` AND (`purchase_invoice_master`.`from_where`<>'SB' AND `purchase_invoice_master`.`from_where`<>'OP')
