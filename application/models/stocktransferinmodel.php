 ...
 */
  /*  public function getOtherCharges($masterId,$totalCost)
     ...
     * @date 13-05-2016
     */
    public function updatePurchaseMaster($dataArray,$do_to_trans) {
     ...
     try {
            ...
            $this->updateDoToTransporter($purchaseId,$do_to_trans);
    ...
    public function updateDoToTransporter($pMastId,$doDtl){
         $this->db->where('purchase_inv_mst_id', $pMastId);
         $this->db->update('do_to_transporter', $doDtl);
     ...
     * @date 13/05/2016
     */
    public function insertNewPurchaseDetail($data, $bagData, $NormalbagData ,$dototrans) {
        $masterDataforupdt = array();
        $BagData = array();
    ...
    public function insertintodoToTransporter($masterId,$detailId,$loc,$dototrans){
        ...
        $data['transporterId'] = $dototrans['transporter_id'];
        $data['chalanNumber'] = $dototrans['challan_no'];
        $data['chalanDate'] = $dototrans['challan_date'];
        $data['is_sent']='Y';
        $data['in_Stock']='Y';
        $data['locationId']=$loc;
    ...
    public function getdotoTransportFromMast($purMasId){
        $data= array();
        $sql = "SELECT `purchase_invoice_master`.`transporter_id`,
                `purchase_invoice_master`.`challan_no`,`purchase_invoice_master`.`purchase_invoice_date`
                FROM `purchase_invoice_master`
                WHERE `purchase_invoice_master`.id=".$purMasId;
        ...
        if ($query->num_rows() > 0) {
            ...
            $data['challan_date'] =$rows->purchase_invoice_date;
  ...
  public function pDetailsInsert($insertId,$searcharray){
      ...
      for($i=0;$i<$countDtl;$i++){
           ...
           // $locationId=0;
            ...
            $this->savedoToTransporter($insertId,$insertdtlId,$searcharray['drplocation'][$i],$searcharray);
  ...
  */ 
public function savedoToTransporter($pMasterId,$pdtlId,$locationId,$detail){
    $session = sessiondata_method();
  /*  echo "PurchaseMasterId = ".$pMasterId;
   ...
   // exit;
    ...
    $data['transporterId']=$detail['transporterid'];
    $data['chalanNumber']=$detail['challanNo'];
    $data['chalanDate'] = date('Y-m-d',  strtotime($detail['receiptDt']));;
    $data['is_sent']='Y';
    $data['in_Stock']='Y';
