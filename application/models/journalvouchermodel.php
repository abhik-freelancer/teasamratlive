...
class journalvouchermodel extends CI_Model{
      ...
      public function getJournalVoucherList($cmpny,$year){
        ...
        $sql="SELECT 
             ...
             `voucher_master`.`cheque_number`,
              ...
              FROM `voucher_master`
              WHERE `voucher_master`.`transaction_type`='JV'
    ...
    public function getJournalMasterDataPdf($voucherMastId){
        ...
        $sql="SELECT 
             ...
             DATE_FORMAT(`voucher_master`.`cheque_date`,'%d-%m-%Y') AS ChqDate 
             FROM `voucher_master`
             WHERE `voucher_master`.`transaction_type`='JV' AND `voucher_master`.`id`='".$voucherMastId."'";
    ...
    public function getVoucherDetaildata($vouchmastId){
       ...
       public function getVoucherNumber($voucherMastId){
        $sql="SELECT `voucher_master`.`voucher_number` FROM `voucher_master` WHERE `voucher_master`.`id`='".$voucherMastId."' AND `voucher_master`.`transaction_type`='JV'";
		$query = $this->db->query($sql);
       if ($query->num_rows() > 0) {
           $row = $query->row();
   ...
   /* public function getAccountByGroupMaster(){
    ...
    public function getjournalVoucherMasterData($voucherMaterId){
        $sql="SELECT 
                `voucher_master`.`voucher_number`,
                ...
                FROM voucher_master
                WHERE `voucher_master`.`id`='".$voucherMaterId."' AND `voucher_master`.`transaction_type`='JV'";
            $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                $data = array(
   ...
   /* 
    ...
    public function getJournalVoucherDetailData($voucherMaterId){
        $sql="SELECT 
   ...
   public function insertJournalVoucherMaster($insertVoucherMaster,$searcharray){
       try {
            $this->db->trans_begin();
            ...
            //exit;
             ...
             // echo($this->db->last_query());exit;
              $this->updateJournalvoucherSerial($insertVoucherMaster);
 ...
 /**
  ...
  public function UpdatejournalVouchr($updateGeneralVoucher,$searcharray){
   ...
   public function updateJournalvoucherSerial($voucherNo){
        $seriallTable=array();
