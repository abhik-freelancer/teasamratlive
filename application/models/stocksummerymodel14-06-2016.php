...
class stocksummerymodel extends CI_Model {
    ...
    public function getStock($groupId,$frmPrice,$toPrice) {
        $data = array();
        $whereClause = '';
         
        if ($groupId == 0) {
            
            $call_procedure = "CALL sp_GetStockWithoutGroupAndCost()";
            
           //CALL sp_allgroup_stock
           // $call_procedure = "CALL sp_allgroup_stock()";
           // $call_procedure = "CALL sp_allgroup_stock(".$frmPrice.",".$toPrice.")";
        }
        if($groupId!=0 && $frmPrice=="" && $toPrice==""){
           // $call_procedure = "CALL sp_groupwise_stock(" . $groupId . ")";
            
            $call_procedure = "CALL sp_GetStockWithGroupWise(" . $groupId . ")";
        }
        if($groupId==0 && $frmPrice!="" && $toPrice!=""){
          //  $call_procedure = "CALL sp_pricewise_allgrp(".$frmPrice.",".$toPrice.")";
            $call_procedure = "CALL sp_GetStockWithCostRangeWise(".$frmPrice.",".$toPrice.")";
          
        }
        if($groupId!=0 && $frmPrice!="" && $toPrice!=""){
           // $call_procedure = "CALL sp_grp_nd_pricewise(".$groupId.",".$frmPrice.",".$toPrice.")";
            
            $call_procedure = "CALL sp_GetStockWithGroupAndCost(".$groupId.",".$frmPrice.",".$toPrice.")";
        }
