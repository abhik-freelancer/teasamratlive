 ...
 */
public function updateOtherCharges($pMasterId,$data=array()){
    try {
          $this->db->trans_begin();
  ...
  /*****old code*******/
    ...
    function getPurchaselistingdata($session) {
        ...
        $sql = "SELECT purchase_invoice_master.`id`,
					...
					HAVING  (purchase_invoice_date BETWEEN '" . date("Y-m-d", strtotime($session['startdate'])) . "' AND '" . date("Y-m-d", strtotime($session['enddate'])) . "') 
					" . $vendorClause . " 
