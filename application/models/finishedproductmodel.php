...
class finishedproductmodel extends CI_Model {
    ...
    public function getPaketDetailForBlend($blendId) {
        $data = array();
        $sql = "SELECT `blending_master`.`id`,`blending_master`.`productid`,`packet`.`packet`,`packet`.`PacketQty`,`packet`.`qtyinBag`,
    ...
    public function getFinishedProductList($comp,$year) {
        $data = array();
        ...
        $sql = "SELECT 
                ...
                WHERE finished_product.`company_id`=".$comp." AND finished_product.`year_id`=".$year."
                GROUP BY 
                `finished_product`.`id`";
    ...
    public function getFinishProductPacketDetails($finishProductId){
        ...
        $sql = "SELECT  
                        ...
                        `product_packet`.`id` as productPacketId,
                        `packet`.`packet`,`packet`.PacketQty,`packet`.qtyinBag
    ...
    public function getFinishProductStock($companyId,$yearId){
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                $data[]=array(
                            "productpacketid"=>$rows->productpacketid,
                            "finishProduct"=>$rows->finishProduct,
