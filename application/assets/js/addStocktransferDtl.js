...
function purchaseDetailsAdd(){
    ...
    var price,transportationCost,group,garden,grade,location,warehouse,numberOfNormalBag,normalBagQty,normalchest,normalBagId;
    ...
    if(validation()){
        ...
        price =$("#txtPrice").val();
        transportationCost = $("#transCost").val();
        group =$("#drpGroup").val();
    ...
    //saving data
    $.ajax({
        url: basepath + "stocktransferin/addNewPurchaseDetails",
        type: 'post',
        data: {
...
function validation(){
   ...
   if(location==0){
       $("#drpLocation").addClass("glowing-border");
        return false;
   }  
