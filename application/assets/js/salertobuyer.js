...
$(function(){
	...
	$(".mini").click(function(){
	 ...
	 $("#saledate").change(function(){
				...
				"Confirm": function() {
					...
					var sample = "<table><tr><td>"+createspan+"<input type='hidden' name='detailsamplename[]' value='"+createspanhidden+"'/></td><td>=></td><td>"+samplenet+"<input type='hidden' name='detailsamplenet[]' value='"+samplenethidden+"'/></td></tr></table>";
						...
						$("#example2").append(
							...
							"<td>"+$('#lot').val()+"<input type='hidden' name='detaillot[]' value='"+$('#lot').val()+"'/><input type='hidden' name='detailtableid[]' value='0'/></td>"+
							"<td>"+$('#invoice').val()+"<input type='hidden' name='detailinvoice[]' value='"+$('#invoice').val()+"' /></td>"+
							"<td>"+ $("#garden option:selected" ).text()+"<input type='hidden'  name='detailgardenid[]' value='"+ $("#garden option:selected" ).val()+"'/><br/>"+ $("#teagroup option:selected" ).text()+"<input type='hidden'  name='detailteagroupid[]' value='"+ $("#teagroup option:selected" ).val()+"'/></td>"+
		...
		$("#addsample").click(function(){
			$.ajax({ 
				    			type: "get", 
                                url: basepath+'salertobuyer/callSamplepage',
                                cache: false, 
                                dataType: 'html',
	  ...
	  $("#stampcharge").blur(function(){
		...
		/*$( ".call" ).change(function() {
		 ...
		 $("#showinvoice").click(function(){
			...
			$.ajax({
			   type: "POST",
			   url:   basepath+"salertobuyer/getlistsalertobuyer",
			   data:  {vendor: vendor,startdate: startdate,enddate: enddate},
			   success: function(data)
			   {
					
						//$("#spinner").hide();
						window.location.href = basepath+'salertobuyer/showlist';
		  ...
		  $(".opendetail").click(function(){
			   ...
			   $.ajax({
			   type: "POST",
			   url:   basepath+"salertobuyer/getlistingindetail",
			   data:  {id: id},
			    success: function(data)
			   {				
						$rowhead = '<table border="0" width="100%" id="popuptable"><thead bgcolor="#CFDBC5" style="font-family:\'Times New Roman\', Times, serif"><td>LOT</th><td>Invoice Number</td><td>Garden</td><td>Grade/<br/>Chest</td><td>GP No/Date</td><td>Package<br/>Stamp/<br/>Net</td><td>Sample</td><td>Gross/<br/>Brokerage</td><td>Total Wt./<br/>Vat</td><td>Price/<br/>Service tax</td><td>Value</td><td>Total</td></thead>'+data+'</table>';
		...
		//$("#ratedropdowntr").show();			
					   ...
					   url:   basepath+"salertobuyer/getTaxlist",
					   success: function(data)
					   {
	...
	function deleterowdb(rowname,rowid,invoice)
	{
		$.ajax({
				type: "POST",
				data:  {detailid: rowid,invoice: invoice},
				url:   basepath+"salertobuyer/deleteInvoicedetail",
				success: function(data)
				{
	...
	function makealltextblank()
	{
		...
		$('#optionrate').val('0');
		$('#optionstax').val('0');
		//$( "#popupwindow" ).closest( "select" ).val('0');
	}
		...
		if(sample != '')
		{
			...
			var tr = '';
			 ...
			 for (i = 0; i < samplearr.length; i++) {
				...
				row = '<table><tr id="sampletr'+rowid+'"><td><input type="text" name="packtno[]" id="packtno" value="'+samplearr[i]+'"/></td><td>&nbsp;</td><td><input type="text" name="packtvalue[]" id="packtvalue" value="'+samplenetarr[i]+'" onblur="calculateCurrentVatratetotal()"/><img src="http://localhost:8080/tea_samrat/application/assets/images/delete.png" id="sampleimg'+rowid+'" height="15" width="15" onclick="deletesample('+rowid+')"></td></tr></table>';
				...
				"Confirm": function() {
					...
					var sample = "<table><tr><td>"+createspan+"<input type='hidden' name='detailsamplename[]' value='"+createspanhidden+"'/></td><td>=></td><td>"+samplenet+"<input type='hidden' name='detailsamplenet[]' value='"+samplenethidden+"'/></td></tr></table>";
						...
						$("#detailtr"+rowid).html(
							...
							"<td>"+$('#lot').val()+"<input type='hidden' name='detailtableid[]' value='"+$('#detailtableid'+rowid).val()+"' /><input type='hidden' name='detaillot[]' value='"+$('#lot').val()+"'/></td>"+
							"<td>"+$('#invoice').val()+"<input type='hidden' name='detailinvoice[]' value='"+$('#invoice').val()+"' /></td>"+
							"<td>"+ $("#garden option:selected" ).text()+"<input type='hidden'  name='detailgardenid[]' value='"+ $("#garden option:selected" ).val()+"'/><br/>"+ $("#teagroup option:selected" ).text()+"<input type='hidden'  name='detailteagroupid[]' value='"+ $("#teagroup option:selected" ).val()+"'/></td></td>"+
							"<td><span name='grade[]'>"+ $("#grade option:selected" ).text()+"</span><br/><span name='ch[]'>"+$('#chfrom').val()+" - "+$('#chto').val()+"</span><input type='hidden'  name='detailgradeid[]' value='"+ $("#grade option:selected" ).val()+"'/><input type='hidden'  name='detailchfrom[]' value='"+ $("#chfrom" ).val()+"'/><input type='hidden'  name='detailchto[]' value='"+ $("#chto" ).val()+"'/></td>"+
							"<td>"+$('#gpno').val()+"<input type='hidden' name='detailgpno[]' value='"+$('#gpno').val()+"' /><br/>"+$('#date').val()+"<input type='hidden' name='detaildate[]' value='"+$('#date').val()+"'/></td>"+
							"<td>"+$('#package').val()+"<input type='hidden' name='detailpackage[]' value='"+$('#package').val()+"'/><br/>"+$('#stamp').val()+"<input type='hidden' name='detailstamp[]' value='"+$('#stamp').val()+"'/><br/>"+$('#net').val()+"<input type='hidden' name='detailnet[]' value='"+$('#net').val()+"'/></td>"+
							"<td>"+sample+"</td>"+
	...
	function deleteSalertobuyer(masterid)
	{
		...
		$.ajax({
				...
				url:   basepath+"salertobuyer/deleteRecord",
				success: function(data)
				{
					$("#row"+masterid).remove();
				}
