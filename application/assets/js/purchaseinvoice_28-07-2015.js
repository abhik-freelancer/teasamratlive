    ...
    if (sample != '')
    {
        ...
        for (i = 0; i < numberOfBagsInBagDetails; i++) {
            ...
            row = '<table class="CSSTableGenerator">' +
                    ...
                    '<option value="3"'+((selectedBags[i]==3)?'Selected=selected':'')+'>Shortage</option>'+
             		'</select>'+
			        '</td>'+
                    '<td>' +
                    '<input type="text" name="packtno[]" id="packtno" value="' + samplearr[i] + '" onblur="calculateCurrentVatratetotal()" onkeypress="checkNumeric(this);"/>' +
                    '</td>' +
                    '<td>&nbsp;</td>' +
                    '<td>' +
                    '<input type="text" name="packtvalue[]" id="packtvalue" class="packtvaluenet" value="' + samplenetarr[i] + '" onblur="calculateCurrentVatratetotal()" onkeypress="checkDecimal(this);"/>' +
                    '</td>'+
                    '<td>'+
            ...
            "Confirm": function() {
                ...
                if (createspan != '')
                {
                    var sample = "<table><tr><td>" +
                            ...
                            "<input type='hidden' name='bagtypeIds[]' id='bagtypeIds"+rowid+"' value='" + bagTypehiddenIds + "'/>"+
                            "</td>"+
                            "<td>"+
                            "<span id='chest"+rowid+"'>"+chestText+"</span>"+
                            "<input type='hidden' name='chestvalues[]' value='" + chestValue + "'/>"+
                            "</td>"+
                            "</tr></table>";
                }
