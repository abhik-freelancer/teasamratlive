...
$(document).ready(function() {
 ...
 $("#saleBillDate").change(function(){
    ...
    $("#addnewDtlDiv").click(function() {
        ...
        $.ajax({
            url: basepath + "finishproductpurchase/createDetails",
            type: 'post',
   ...
   $("#saveSaleBill").click(function(){
      ...
      if(getValidation()){
          ...
          if(detailvalidation ()){
           ...
           $.ajax({
            url: basepath + "finishproductpurchase/saveData",
            type: 'post',
                ...
                if(data==1){
                                        $( "#sale_bil_save_dilg" ).dialog({
                                                 modal: true,
                                                    buttons: {
                                                Ok: function() {
                                                     window.location.href = basepath + 'finishproductpurchase';
                                                    $( this ).dialog( "close" );
                                                }
                                              }
                                            });
 ...
 $(document).on('change','.vendor',function(){
    var vendorId = $("#vendor").val();
     ...
     $.ajax({
            url :  basepath + "finishproductpurchase/getCreditDaysFromCustomer",
            type: "POST",
            dataType:'json',
 ...
 $(document).on('blur','.vendor',function(){
  ...
  /*$(document).on('blur','.salebillNo',function(){
    var SaleBillNo = $("#txtSaleBillNo").val();
     ...
     $.ajax({
            url :  basepath + "finishproductpurchase/checkExistingSaleBillNo",
            type: "POST",
            dataType:'json',
 ...
 // Due Date
    ...
    $(document).on('change', ".finalProduct", function() {
        ...
        $.ajax({
            url :  basepath + "finishproductpurchase/get_final_product_rate",
            type: "POST",
            dataType:'json',
...
function getValidation(){
   ...
   if(vendorId==0){$("#vendor").addClass('glowing-border');return false;}
