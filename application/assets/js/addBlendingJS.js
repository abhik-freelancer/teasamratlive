...
//lot dropdown change
 ...
 $(document).on('blur', "#txtBlendingRef", function() {
      var refrenceNo = $("#txtBlendingRef").val();
     // alert(refrenceNo);
    $.ajax(
                    {
                        type: 'POST',
                        dataType: 'json',
                        url: basepath + "blending/checkDuplicateRefrenceNo",
                        data: {refrenceNo: refrenceNo},
                        success: function(data) {
                                if(data==1){
                                   $("#dialog-for-refrenceno").dialog({
                                   resizable: false,
                                   height: 140,
                                   modal: true,
                                   buttons: {
...
function blendePrice(id){
    ...
    var netperkg = ($("#hdnetBag_"+id).val()==""?0:$("#hdnetBag_"+id).val());
    
    blended_price = parseFloat(rate) * parseFloat(usedBag) * parseFloat(netperkg);
    
    $("#txtBlendedPrice_"+id).val((blended_price).toFixed(2));
...
function getAverageBlendedPrice(){
   var totalBlendedPrice = 0;
    ...
    var totalKgsBlended=0;
    
     totalBlendedPrice= getTotalBlendedPrice();
     totalBlendedBags = getTotalBlendedBag();
     totalKgsBlended = getTotalBlendedKgs();
     if(totalBlendedBags!=0){
      avgBlendedPrice = (totalBlendedPrice)/(totalKgsBlended);
