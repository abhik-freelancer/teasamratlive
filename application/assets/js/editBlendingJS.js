...
function blendePrice(id){
    ...
    var netperbag = ($("#hdnetBag_"+id).val()==""?0:$("#hdnetBag_"+id).val());
    
    blended_price = parseFloat(rate) * parseFloat(usedBag) * parseFloat(netperbag);
...
function getTotalBlendedPrice(){
    ...
    var totalBlendedPrice=0;
     ...
     $('input[name^="txtBlendedPrice"]').each(function() {
        var bprice = parseFloat(($(this).val() == "" ? 0 : $(this).val()));
        totalBlendedPrice =parseFloat(totalBlendedPrice + bprice);
        //console.log("totalBlendedBag "+i+":"+bag);
        //i++;
    });
...
function getAverageBlendedPrice(){
   var totalBlendedPrice = 0;
    ...
    totalblendedKgs = getTotalBlendedWeight();
  
    
     if(totalBlendedBags!=0){
      avgBlendedPrice = (totalBlendedPrice)/(totalblendedKgs);
