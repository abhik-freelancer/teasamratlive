...
<section id="loginBox" style="height: 170px;">
    <table width="90%" >
        <tr>
            <td>Ref.No.&nbsp;</td>
            <td>
                <input type="hidden" name="txtVendorAdjustmentId" id="txtVendorAdjustmentId" value="<?php echo($bodycontent['vendoradjustment']['AdjustmentId']); ?>"/>
                <input type="text" name="txtRefNo" id="txtRefNO" class="textStyle" 
                       value="<?php echo($bodycontent["vendoradjustment"]["AdjustmentRefNo"]) ?>"/>
            </td>
            <td>Date&nbsp;</td>
            <td><input type="text" name="dateofadjustment" id="dateofadjustment" class="textStyle datepicker" 
                       value="<?php echo($bodycontent["vendoradjustment"]["DateOfAdjustment"]) ?>"/></td>
        </tr>
        ...
        <tr>
            <td>Vendor&nbsp;</td>
            <td>
                <select name="vendor" id="vendor" class="selectStyle custom-select" <?php if ($bodycontent['vendoradjustment']['AdjustmentId']){echo("disabled='disabled'");} ?>>
                    <option value="">--Select--</option>
                    <?php foreach ($header['vendors'] as $rows) { ?>

                        <option value="<?php echo($rows["vendoraccountId"]); ?>" 
                        <?php if ($rows["vendoraccountId"] == $bodycontent['vendoradjustment']['vendorAccId']) {
                            echo('selected');
                        } else {
                            echo('');
                        }
            ...
            <td>
                <div id="advancevchResult">
                        ...
                        <?php
                            if ($bodycontent['vendoradjustment']['AdjustmentId']) {

                                //var_dump($bodycontent['advancevoucher']);
                                //echo($bodycontent['vendoradjustment']['advanceMasterId']);
                                ?>
                                ...
                                <select id="advanceVoucher" name="advanceVoucher" class="selectStyle" style="width:200px;">
                                    ...
                                    <option value="<?php echo($rows["advanceId"]); ?>"  
                                        <?php if($bodycontent['vendoradjustment']['advanceMasterId']==$rows["advanceId"]){echo("selected='selected'");} ?> >
                                            <?php echo($rows["voucher"]); ?>
                                        </option>
        ...
        <tr>
            <td>Advance </td>
            <td>
                <input type="text" id="advanceamount" name="advanceamount" class="textStyle" readonly="readonly" 
                       value="<?php echo($bodycontent['remaining']) ?>"/>
            </td>
            <td>Total</td>
            <td><input type="text" id="totalAdjustedAmount" name="totalAdjustedAmount" 
                       class="textStyle" readonly="readonly" value="<?php echo($bodycontent['vendoradjustment']['TotalAmountAdjusted']); ?>"/></td>
        </tr>
            ...
            <td style="padding:0 15px 0 15px;">
                <div id="billList">
                    <select id="purchaseBill" name="purchaseBill" style="width:300px;" class="selectStyle">
                         ...
                         <?php if($bodycontent['vendoradjustment']['AdjustmentId']){
...
<section id="loginBox">
    <table id="billAdjustTable" width="60%" class="gridtable" align="center">
        ...
        <tbody>
            <?php
            if ($bodycontent["vendorAdjstDtl"]) {

                foreach ($bodycontent["vendorAdjstDtl"] as $value) {

                    //echo($value["vendAdjstDtlId"]);
                    ?>

                    <tr>
                        <td><?php echo($value["invoiceNo"]); ?><input type='hidden' name=<?php echo($value["vendorBillMasterId"]); ?> value="<?php echo($value["vendorBillMasterId"]); ?>"/></td>
