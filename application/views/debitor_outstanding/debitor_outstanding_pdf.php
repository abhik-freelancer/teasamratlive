<html>
    ...
    <body>
       ...
       <table width="100%" class="demo">
           ...
           if ($debitoroutstanding){
               foreach($debitoroutstanding as $debitor_due){
                    $pTotalOpening+=$debitor_due['opening'];
                    $pTotalDebit+=$debitor_due['debitAmt'];
                    $pTotalCredit+=$debitor_due['creditAmt'];
                    $pTotalBlnc+=$debitor_due['balance'];
                   
                   ?>
           <tr>
               <td align="left"><?php echo $debitor_due['accountname'];?></td>
               <td align="right"><?php if($debitor_due['opening']==0){echo "";}else{ echo number_format($debitor_due['opening'],2);}?></td>
               <td align="right"><?php if($debitor_due['debitAmt']==0){echo "";}else{echo number_format($debitor_due['debitAmt'],2);}?></td>
               <td align="right"><?php if($debitor_due['creditAmt']==0){echo "";}else{echo number_format($debitor_due['creditAmt'],2);}?></td>
               <td align="right"><?php echo number_format($debitor_due['balance'],2);?></td>
               <td align="left"><?php echo $debitor_due['balancetag'];?></td>
           </tr>
           <?php 
                 $lncount = $lncount+1;
                 if($lncount>32){?>
        ...
        <table width="100%" class="demo">
           ...
           <?php } ?>
            ...
            <?php  
            $totalOpening = $totalOpening+$debitor_due['opening'];
            $totalDebit = $totalDebit+$debitor_due['debitAmt'];
            $totalCredit = $totalCredit+$debitor_due['creditAmt'];
            $totalBalance = $totalBalance+$debitor_due['balance'];
               }
               }
           ?>
           <tr>
