...
class purchaseinvoice extends CI_Controller {
    ...
    public function updatePurchaseDetail() {
        ...
        $TBcharges = $this->input->post('TBcharges');//27-09-2016
  ...
  //  echo ('totalCost :'.$totalCost);
   ...
   /* echo('<pre>');
    ...
    public function addNewPurchaseDetails() {
        ...
        $gross = $this->input->post('gross');
        $brokerage = $this->input->post('brokerage');
        $TbCharges = $this->input->post('TBcharges');
        $gpNumber = $this->input->post('GPnumber');
        $gpDate = ($this->input->post('gpDate') == "" ? NULL : date("Y-m-d", strtotime($this->input->post('gpDate'))));
        $price = $this->input->post('price');
        ...
        $doTranporterdata = $this->GetdataForDoToTransporter($purchaseMasterId);
        
        $do_transport_insert['transporterId'] = $doTranporterdata['transporter_id'];
        $do_transport_insert['chalanNumber'] = $doTranporterdata['challan_no'];
        $do_transport_insert['chalanDate'] = $doTranporterdata['challan_date'];
       ...
       exit;*/
        ...
        $update = $this->purchaseinvoicemastermodel->insertNewPurchaseDetail($data,$purchType, $bagData, $NormalbagData,$vdata,$do_transport_insert);
    ...
    public function GetdataForDoToTransporter($pMastId){
        $result = $this->purchaseinvoicemastermodel->GetdataForDoToTransporter($pMastId);
        return $result;
    }
