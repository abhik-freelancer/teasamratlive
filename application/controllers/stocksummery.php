...
class Stocksummery extends CI_Controller {
  ...
  /*  public function getStockPrint(){
       ...
       foreach ($result['Groupwise'] as  $value) {
           ...
           $groupReport[$value['group_code']]=$this->stocksummerymodel->getStock($value['teagroup_master_id'],$fromPrice,$toPrice,$companyId);
           $this->db->freeDBResource($this->db->conn_id); 
       }
  ...
  /* }*/
   ...
   public function getStockPrint(){
     ...
     /* foreach ($result['Groupwise'] as  $value) {
           ...
           $groupReport[$value['group_code']]=$this->stocksummerymodel->getStock($value['teagroup_master_id'],$companyId);
           $this->db->freeDBResource($this->db->conn_id); 
       }
       */
               $result['stock'] = $this->stocksummerymodel->getStock($groupId,$fromPrice,$toPrice,$companyId);
    ...
    foreach($result['stock'] as $rows)
    {
       /*  if($lnCnt>60){
        ...
        $str='<tr> <td>'.$rows['Location'].'---'.$lnCnt.'</td>'
                            ...
                            .'<td align="right">'.number_format($rows['amount'], 2).'</td></tr>';
