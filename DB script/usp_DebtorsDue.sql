ALTER PROCEDURE usp_DebtorsDue (
yearId INT,
companyId int,
fromdate date,
todate date)
...
SELECT 
  financialyear.`start_date` INTO fiscalStartDate 
FROM
  `financialyear` 
...
FETCH NEXT FROM cursor_accountId INTO accountId,accountDesc;
 ...
 SELECT IFNULL(`account_opening_master`.`opening_balance`,0) INTO openingBal FROM `account_opening_master` 
 ...
 IF fromdate > fiscalStartDate 	THEN
	# debit
	SELECT  IFNULL(SUM(VD.`voucher_amount`),0) INTO totalDebit FROM 
	voucher_detail VD 
  ...
  # credit
  SELECT  IFNULL(SUM(VD.`voucher_amount`),0) INTO totalCredit FROM 
	voucher_detail VD 
...
END;
#call usp_DebtorsDue (6,1,'2016-04-01','2016-09-15')