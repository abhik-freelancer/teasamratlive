     ...
     /*   $sql = "SELECT 
        ...
        if ($query->num_rows() > 0) {
           ...
           /* $data['purchaseAmt'] = $rows->total_tea_value+$rows->total_brokerage+$rows->total_service_tax+
            ...
            $data['purchaseAmt'] = $rows->tea_value+$rows->brokerage+$rows->service_tax+$rows->total_cst+$rows->stamp+$rows->other_charges+
                                    $rows->round_off;
   ...
   /* public function updatePurchaseMaster($dataArray) {
    ...
    public function updateVendorBillMaster($purchaseInvoiceMasterId,$dataArray){
        ...
        $fieldToUpdate =array(
            ...
            "vendorAccountId"=>$this->getAccountIdByVendorId($dataArray["vendor_id"])
        );
...
/* public function insertNewPurchaseData($pMaster,$searcharray){
  ...
  public function InsertVendorBillMaster($insertId,$searcharray){
        ...
        $vendorBillMaster['vendorAccountId'] = $this->getAccountIdByVendorId($searcharray['vendor']);
...
/**
         ...
         * @date 18/08/2016
         */
        public function getAccountIdByVendorId($vendorId){
...
public function saveDataDoToTranspoerter($pMasterId,$pdtlId,$location_Id,$searcharray){
    ...
    $data['chalanDate'] = date('Y-m-d',  strtotime($searcharray['challanDate'])); 
    $data['is_sent']='Y';
    $data['in_Stock']='Y';
